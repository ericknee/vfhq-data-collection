...
def download_and_extract_frames(celebrity_names, download_path='videos', max_videos_per_celebrity=20, desired_fps=6):
    if not os.path.exists(download_path):
        os.makedirs(download_path)
    ...
    ydl_opts = {
        'format': 'bestvideo[height=2160][ext=mp4]+bestaudio/best[ext=m4a]/best',
        'noplaylist': True,
        'quiet': True,
        'merge_output_format': 'mp4',
    }
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            try:
                ...
                for video in results:
                    ...
                    try:
                        # Extract information for the specific video format
                        info_dict = ydl.extract_info(video_url, download=False)
                        formats = info_dict.get('formats', [])
                        video_format = next((f for f in formats if f.get('height') == 2160 and f.get('vcodec') != 'none'), None)

                        if video_format and info_dict.get('duration', 0) >= 60:
                            # Use the format ID to get the 4K video URL
                            video_url_4k = video_format['url']

                            prefix = convert_filename(f"{name}_{info_dict['id']}")
                            output_folder = os.path.join(download_path, prefix)
                            ...
                            print(f"Skipping {video_url}, does not meet criteria (4K video, and longer than 60 seconds)")
...
download_and_extract_frames(celebrities)
